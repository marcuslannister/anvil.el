...
#+date: 2026-04-15
#+property: status PHASE-4A-SHIPPED

* STATUS
~Phase 1+2+3+4a SHIPPED 2026-04-16~ — lane split + ~:kind~ + classifier +
batch warmup + ~anvil-worker--metrics-latency~ per-lane plist
(~:samples :spawn-ms-sum :wait-ms-sum :total-ms-sum :totals~) と
~latency-metrics-show~ (n / mean / p50 / p99) + 47 worker ERT。
4b: 案 B (worker 側 persistent listener) は 4a baseline 取得後に判断。
