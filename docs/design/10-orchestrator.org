...
* STATUS
~Phase 1a + 1b SHIPPED~ (2026-04-18)
...
- Phase 1a (commit 45468b2): provider abstraction, claude provider,
  ...
  pass
- Phase 1b (commit TBD, branch feat/orchestrator-phase-1b): git
  worktree isolation (claude native + anvil-side fallback),
  =:depends-on= DAG with cycle / unknown-name detection +
  propagating failure semantics, stdout/stderr head + tail
  truncation at the 1 MB cap. ERT +12 (= 30 pass / 1 live skip)

Phase 2 (aider middleware), 2b (anvil-cron), 3/3b (gemini /
ollama native), 4 (consensus) は design 残し。Review Questions
Q1-Q12 resolved 状態キープ。
