...
* STATUS
~Phase 1+2(full) SHIPPED~ (2026-04-16) — Phase 1 helpers に加え、
全 mutating ~file-*~ tool に ~:warnings~ を組込:
~file-read~, ~file-replace-string~, ~file-replace-regexp~, ~file-insert-at-line~,
~file-delete-lines~, ~file-append~, ~file-prepend~, ~file-batch~,
~file-ensure-import~ (~file-batch-across~ は内部で ~file-batch~ を呼ぶため
per-file result に自動伝播)。
