...
TTL cache keyed on URL + auth preset.  Measurement (see
[[file:benchmarks/results/http-browser-report-2026-04-19.org][=http-browser-report-2026-04-19.org=]]):
within the default TTL, *second and subsequent fetches cost 0 ms
wallclock and send zero bytes over the network* — the response
plist's =:from-cache= is =t=.  An agent that fetches the same
Reddit thread / GitHub PR / docs page twice during a session pays
for it exactly once.

Separately, =anvil-browser= returns an accessibility-tree snapshot
instead of raw HTML — measured *7–9× smaller* than the same URL's
HTTP body on plain pages, and routinely 40–60× smaller on heavy
rendered pages (Reddit threads, Jira boards).  Smaller payload
means smaller prompt when the agent quotes the fetch back to the
LLM.
