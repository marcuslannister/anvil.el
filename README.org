...
Rust runtime called NeLisp) as 40+ tools an AI agent can call (default install; ~150 with all optional modules enabled).
...
| Repo | Role | Required? |
|------+------+-----------|
| [[https://github.com/zawatton/anvil.el][anvil.el]]      | The AI brain — MCP server, ~40 default tools (+~110 via optional modules) | yes |
...
Anvil's v1.0.0 surface ranges from *~40 tools* in the default install to *~150 tools* when commonly used optional modules (memory / orchestrator / session / http / …) are enabled
...
Anvil exposes ~40 default + ~110 optional named primitives (~150 total when fully loaded), each registered with an explicit
